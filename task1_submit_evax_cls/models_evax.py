...
def build_eva_x_small(img_size=1024, in_chans=1, pretrained_path=None):
    """EVA-X small backbone. eva_x.py의 eva_x_small_patch16과 동일한 설정.

    dynamic_img_size=True: 224로 사전학습된 pos embed / RoPE를 임의 해상도로
    자동 resample (timm Eva._pos_embed 내부의 resample_abs_pos_embed).
    num_classes=0: classification head 제거 (우리는 feature만 사용).
    """
    model = Eva(
        img_size=img_size,
        patch_size=16,
    ...
    if pretrained_path:
        ...
        print("  [WARN] pretrained_path 없음 — random init")
