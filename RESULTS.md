...
| 5 | λ=0.7 (되돌림) | 0.6774 | — | 0.5244 |
| 6 | 5-fold ensemble (logit 평균) | 0.6906 | **0.1979** | 0.5428 |

3~4번이 본 저장소 코드의 최고 기록이다. 팀 최종 제출은 공동 참가자가 개발한
다중 백본 융합 모델(final 0.5838)이며, 그 방법론은 본 저장소 범위 밖이다.
...
**5-fold ensemble은 손해였다.** 555장(train+val)을 5분할해 학습한 모델들의
분류 logit을 평균했더니 detection이 0.7089 → 0.6906으로 떨어졌다. fold별
검증 성능 편차가 컸고(0.6572~0.7231), 약한 fold가 평균을 끌어내려 present
판정이 줄어든 것으로 보인다. 다만 Dice는 0.1667 → **0.1979**로 올라 당시
리더보드 전체에서 최고를 기록했는데, present 판정이 줄면서 애매한 케이스가
평균에서 빠진 결과로 해석된다.
...
| **5-fold ensemble** | external 0.5428 | 위 external 절 참조 |
...
2. **학습 데이터 확대.** 상위권 팀들은 외부 공개 데이터(NLM Shenzhen 등)를
   추가해 detection을 올렸다. 본 저장소는 챌린지 제공 데이터만 사용했다.
3. **강건성 기반 모델 선택.** internal 점수가 external을 예측하지 못한다면,
   변형(회전·스케일·해상도·강도·블러)에 대한 예측 안정성을 선택 기준으로
   쓰는 편이 낫다. 챌린지 종료 시점에는 검증하지 못했다.
