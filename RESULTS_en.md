...
| 5 | λ=0.7 (reverted) | 0.6774 | — | 0.5244 |
| 6 | 5-fold ensemble (logit averaging) | 0.6906 | **0.1979** | 0.5428 |

Rows 3–4 are the best results from the code in this repository. The team's
final submission was a multi-backbone fusion model developed by a collaborator
(0.5838), which is outside the scope of this repository.
...
**The 5-fold ensemble was a net loss.** We split the combined 555 training and
validation images into five folds and averaged the classification logits, but
detection fell from 0.7089 to 0.6906. Fold-wise validation performance varied
widely (0.6572–0.7231), and the weaker folds appear to have pulled the average
down, reducing the number of positive predictions. Dice rose to **0.1979** —
the highest on the leaderboard at that time — which we read as a side effect:
borderline cases dropped out of the average along with their low Dice values.
...
| **5-fold ensemble** | External 0.5428 | See the external section above |
...
1. **The 12 mislocalized cases.** Every zero-Dice case comes from this group.
   Distinguishing cavities from other lucent lesions within the lung fields was
   not solved by preprocessing, resolution, or backbone size, and an ROI second
   stage would not address it either.
2. **Larger training sets.** Top-ranked teams added public external data (e.g.
   the NLM Shenzhen set) to improve detection. This repository uses only the
   data provided by the challenge.
3. **Robustness-based model selection.** If internal scores do not predict
   external behaviour, prediction stability under perturbation (rotation,
   scale, resolution, intensity, blur) is a more promising selection criterion.
   We were unable to validate this before the challenge closed.
