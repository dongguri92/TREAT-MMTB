...
조직위 external set. 한국·몽골·페루·필리핀 등 5개 국제 기관 데이터.

| # | 제출 구성 | detection | Dice | final |
|---|---|---|---|---|
| 1 | EVA-X λ=0.5 + segmentation veto (예선 제출본) | 0.6713 | 0.1568 | 0.5170 |
| 2 | λ=0.1 + classification-driven (veto 제거) | 0.6862 | 0.1644 | 0.5296 |
| 3 | **+ percentile 정규화** | 0.7063 | 0.1657 | **0.5441** |
...
(51% 유지)로 떨어졌다. 리더보드 상위권 전체가 Dice 0.05~0.20 구간에 몰려
...
**Internal 점수차가 external을 예측하지 못한다.** 네 번의 변경 중 세 번에서
internal과 external의 효과가 **부호까지 반대**였다.

| 변경 | internal Δ | external Δ |
|---|---|---|
| veto 제거 | **−0.0161** | **+0.0126** |
| percentile 정규화 | **−0.0053** | **+0.0145** |
| classification TTA | **−0.0077** | **+0.0022** |
| λ 0.1 → 0.7 | +0.0074 | **−0.0197** |

따라서 internal 리더보드 순위로 external 제출을 고르는 것은 신뢰할 수 없었고,
"어느 구성 요소가 취약할 때 전체가 함께 무너지는가" 같은 메커니즘 수준의
논거가 더 유효했다. 실제로 채택한 세 변경(veto 제거, percentile, TTA)은 모두
internal 점수가 낮아졌음에도 메커니즘 논거로 선택했고, 세 번 모두 external에서
개선되었다.
...
| + classification TTA (external 제출본) | 0.8829 | 0.2948 | 0.7065 |
...
```
present = cls_prob >= 0.5            # cls_prob은 원본/좌우반전 view의 평균
...
TTA는 **분류 헤드에만** 적용한다. 마스크를 여러 view로 평균하면 경계가
흐려져 Dice가 어디로 움직일지 예측하기 어렵고, 본 시스템의 Dice는 이미
참가팀 중 상위권이었기 때문이다.
...
파이프라인(좌표 3중 역변환, 성분별 병합)은 시간 제약으로 구현하지 않았으며,
관련 코드도 저장소에 포함하지 않았다.

---

## Negative results

같은 시도를 반복하지 않도록 기록. **아래 모든 축에서 internal Dice가
0.26~0.31을 벗어나지 못했다.**
...
| **cls head 강화** (hidden layer 추가) | 하락 | |
...
| **강한 augmentation** (CLAHE clip 랜덤 + 열화 확률 ↑) | 0.8829 / 0.2984 / 0.7075 | detection 1건 하락 |
...
DICOM window와 달리 **모든 영상에 동일하게 적용**되므로 커버리지 문제가 없다.
...
internal 이득은 작았으나 external에서 +0.0145로 가장 큰 단일 개선이었다.
internal 111건은 상대적으로 균질하지만 external은 5개 기관 데이터라 극단값
편차가 훨씬 크기 때문으로 해석된다.
