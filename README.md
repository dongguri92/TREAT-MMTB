...
팀 최종 제출(final 0.5838)은 공동 참가자가 개발한 다중 백본 융합 모델이며
본 저장소 범위 밖이다.
...
> external에서 개선되었다. 자세한 내용은 RESULTS.md 참조.
...
| 파일 | 역할 |
|---|---|
| `main.py` | 학습 진입점. 단일 split / k-fold / 전체 데이터 학습 지원 |
...
| `datasets.py` | 전처리 파이프라인, k-fold 분할, 전체 데이터 로더 |
...
### k-fold / 전체 데이터 학습
```bash
# train+val 555장을 5분할, fold 0을 검증으로
python main.py --model evax_seg --fold 0 --n_folds 5 --tag evax_fold0 ...

# train+val 555장 전부를 학습에 사용 (held-out 없음, epoch 고정 필요)
...
강한 augmentation, 5-fold ensemble** 모두 개선을 주지 못했다.
